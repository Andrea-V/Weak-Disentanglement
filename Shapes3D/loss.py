...
class RelationalLossCompute(nn.Module):
	...
	def __call__(self, z_true_more_hue, z_true_less_hue, z_true_bigger, z_true_smaller, z_true_shape, z_more_hue, z_less_hue, z_bigger, z_smaller, z_shape, retain_graph=False):

		loss_more_hue = self.criterion(z_true_more_hue, z_more_hue)
		loss_less_hue = self.criterion(z_true_less_hue, z_less_hue)
		loss_bigger = self.criterion(z_true_bigger, z_bigger)
		loss_smaller = self.criterion(z_true_smaller, z_smaller)
		loss_shape = self.criterion(z_true_shape, z_shape)

		loss = self.cfg.loss.prior_weight * (loss_more_hue + loss_less_hue + loss_bigger + loss_smaller + loss_shape) / 5.
