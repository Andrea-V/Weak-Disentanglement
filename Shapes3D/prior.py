...
class AdaptiveRelationalPrior(nn.Module):
		...
		def _init_mog_adaptive_prior(self, cfg, tr_set, model):
			...
			for i, (x, y) in enumerate(tr_set):
				...
				xs.append(x.reshape((-1, 3, 64, 64)).cpu())
		...
		def forward(self, batch):
			more_hue_inputs = torch.cat([batch.more_hue_sample, torch.tensor([1, 0, 0, 0, 0])], dim=-1).to(self.cfg.device)
            less_hue_inputs = torch.cat([batch.less_hue_sample, torch.tensor([0, 1, 0, 0, 0])], dim=-1).to(self.cfg.device)
            bigger_inputs = torch.cat([batch.bigger_sample, torch.tensor([0, 0, 1, 0, 0])], dim=-1).to(self.cfg.device)
            smaller_input = torch.cat([batch.smaller_sample, torch.tensor([0, 0, 0, 1, 0])], dim=-1).to(self.cfg.device)
            shape_inputs = torch.cat([batch.shape_sample, torch.tensor([0, 0, 0, 0, 1])], dim=-1).to(self.cfg.device)
               
			z_more_hue_pred = self.relation_layer(batch.more_hue_inputs)
			z_less_hue_pred = self.relation_layer(batch.less_hue_inputs)
			z_bigger_pred = self.relation_layer(batch.bigger_inputs)
			z_smaller_pred = self.relation_layer(batch.smaller_input)
			z_shape_pred = self.relation_layer(batch.shape_inputs)

			return z_more_hue_pred, z_less_hue_pred, z_bigger_pred, z_smaller_pred, z_shape_pred
